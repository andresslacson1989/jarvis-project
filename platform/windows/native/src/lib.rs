...
pub enum NativeErrorKind {
    ...
    PathAliasProbeFailed,
    MaintenanceHeld,
    MaintenanceAlreadyHeld,
    MaintenanceLockBusy,
    NormalHeld,
...
impl std::fmt::Display for NativeError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(match self.kind {
            ...
            NativeErrorKind::PathApiFailure => {
                "native path identity API failed or returned invalid data"
            }
            NativeErrorKind::PathIdentityMismatch => {
                "native path identity did not match the expected object"
            }
            NativeErrorKind::PathAliasProbeFailed => {
                "native path alias identity could not be verified"
            }
            NativeErrorKind::MaintenanceHeld => "JARVIS maintenance authority is already held",
            NativeErrorKind::MaintenanceAlreadyHeld => {
                "JARVIS maintenance authority is already held by this owner"
            }
            NativeErrorKind::MaintenanceLockBusy => {
                "JARVIS maintenance authority is held by another owner"
            }
