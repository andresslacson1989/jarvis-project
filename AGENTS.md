...
When implementing or reviewing platform/runtime boundaries, backup/recovery cryptography, project-policy trust, supply-chain/TUF signing, Codex provider qualification, custody/evidence records, or shipped-desktop visual verification, read `docs/agent-instructions/security-runtime-provider-and-records.md` in full before acting.
...
When performing application implementation, subsection verification, checkpoint work, evidence recording, or completion/status decisions, read `docs/agent-instructions/implementation-execution-protocol.md` in full before acting.
