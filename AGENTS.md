...
- Publish every GitHub `origin` branch and tag in the repository publication scope first; verify each GitHub ref and exact commit, then mirror the same ref and SHA to the GitLab remote with non-force, non-delete pushes. Never publish to GitLab before the corresponding GitHub publication, and never force-update, delete, or rewrite a conflicting GitLab ref; stop and obtain owner/governance resolution instead. This is mandatory repository synchronization only: it is separate from GitLab runner registration, project/variable/configuration changes, and CI qualification, and it does not grant GitLab CI authority or override the active contract. An operational hold that explicitly prohibits repository mirroring must be honored until the owner/governance resolution is recorded; it must not be silently waived or replaced with local/CI evidence.
