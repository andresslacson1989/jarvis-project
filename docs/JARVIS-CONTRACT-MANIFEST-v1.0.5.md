# JARVIS Contract Manifest v1.0.5
...
The current suite is governed by `docs/JARVIS-IMPLEMENTATION-CONTRACT-v1.0.5.md`.
...
|---|---|---:|---|
| 1 | `docs/JARVIS-IMPLEMENTATION-CONTRACT-v1.0.5.md` | 1.0.5 | top-level product/architecture/security/production contract |
...
| 14 | `docs/implementation/JARVIS-VERIFICATION-RELEASE-CONTRACT.md` | 1.0.4 | central release evidence plus all cumulative active-contract gates |
| 15 | `docs/implementation/JARVIS-IMPLEMENTATION-PLAN.md` | 1.0.5 | implementation sequencing including early voice feasibility and closure proofs |
...
Several unchanged component revisions retain historical header text naming their earlier parent suite. Their inclusion in the current suite is governed exclusively by this manifest and the v1.0.5 top-level contract. Those historical header references do not make superseded top-level contracts current and do not create an overlay.

The v1.0.5 closure contracts are cumulative specializations of compatible broad rules in the inherited Data/Security/Operations/Verification components. Where a broad inherited rule allows implementation choice, the corresponding v1.0.5 specialized contract fixes the production choice. The specialized rule is part of the current suite, not an ADR override.

---

# 3. CURRENT DECISION BOUNDARY

The suite incorporates the accepted current effects of ADRs through **ADR-073**.
...
**END — JARVIS CONTRACT MANIFEST v1.0.5**
