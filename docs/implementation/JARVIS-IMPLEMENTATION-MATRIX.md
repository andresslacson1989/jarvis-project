...
|---|---|---|---|---:|---|---|
| **SECTION 0 — Repository / Platform Contracts / Toolchain / Governance** | **VERIFIED** | — | `Phase 0`; checkpoint: Repository Governance + Platform Boundary + Contract-Drift Protection Ready | 10 CA / 8 scoped | None within Section 0 scope; hosting residual risk remains truthfully recorded under compensating controls | Corrected candidate `7221d70b...` passed exact-head static-ci run `31988167248`; evidence reconciliation integrated non-force as `bc82d724f511cbd7d57364760223fa03b66af660`; final authoritative master static-ci run `31988970480` passed on that exact SHA; see `docs/implementation/evidence/0.CP-phase0-checkpoint.md` |
...
| Section / Subsection | Status | Depends On | Governing Contract / Traceability | Score | Current Gap | Evidence / Result |
|---|---|---|---|---:|---|---|
| **SECTION 1 — Windows Tauri Host / Mission Control Foundation / Application-Owned Core** | **IN PROGRESS** | 0.CP | `Phase 1`; checkpoint: Windows Desktop Trust + Mission Control Foundation Ready | — | 1.1 is the only active subsection; later Section 1 work remains not started | Active candidate branch `codex/section1-1.1-windows-resource-fix`; candidate `6f1123aa...`; CI run `31998608356` reached the Windows build but failed on unavailable `llvm-rc`; exact-SHA CI remains outstanding |
| ↳ **1.1** Tauri 2 + React desktop workspace with bundled-local authoritative WebView | **IN PROGRESS** | 0.CP | PLAN §3; RT §§2–5; RP §3; UI §8 | — | ICO resource/checker/tests pass; CI resource compiler provisioning and literal exact-candidate checkout remain to be qualified; independent audit and integration remain outstanding | Candidate `6f1123aa...`; focused ICO tests, desktop foundation, UI build, Core/UI gates, and local Phase 0 check pass; CI `31998608356` failed at `tauri-winres` with `NotAttempted("llvm-rc")`; next candidate must install/verify `llvm-rc` and check out the PR head SHA rather than a synthetic merge |
