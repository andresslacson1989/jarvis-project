...
|---|---|---|---|---:|---|---|
| **SECTION 0 — Repository / Platform Contracts / Toolchain / Governance** | **IN PROGRESS** | — | `Phase 0`; checkpoint: Repository Governance + Platform Boundary + Contract-Drift Protection Ready | 9 CA / 8 scoped | Separate Core and UI compilation is now implemented and gated; exact candidate CI, independent review, and non-force integration remain | `0.CP` was reopened to reconcile the literal clean-checkout exit requirement; current candidate adds separate `build:core` and `build:ui` gates |
...
| ↳ **0.CP** SECTION CHECKPOINT — Phase 0 clean-checkout, governance, platform-boundary, and drift proof | **IN PROGRESS** | 0.1–0.13 | PLAN §2 Exit; PLAN §26 checkpoint; VR §§6,9,33 | 9 CA / 8 scoped | Candidate now enforces independent Core/UI builds; remote exact-SHA CI and review remain pending | `0.CP` reopened for literal-exit reconciliation; separate `pnpm build:core` and `pnpm build:ui` pass locally on the master-based candidate |
