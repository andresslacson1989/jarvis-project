...
| Section / Subsection | Status | Depends On | Governing Contract / Traceability | Score | Current Gap | Evidence / Result |
|---|---|---|---|---:|---|---|
| **SECTION 1 — Windows Tauri Host / Mission Control Foundation / Application-Owned Core** | **IN PROGRESS** | 0.CP | `Phase 1`; checkpoint: Windows Desktop Trust + Mission Control Foundation Ready | — | 1.1 is the only active subsection; later Section 1 work remains not started | Active candidate branch `codex/section1-1.1-windows-resource-fix`; candidate `297e9b4...`; native Windows prerequisite and exact-SHA CI are pending |
| ↳ **1.1** Tauri 2 + React desktop workspace with bundled-local authoritative WebView | **IN PROGRESS** | 0.CP | PLAN §3; RT §§2–5; RP §3; UI §8 | — | ICO validation, exact checkout, host checks, and pre-Windows gates pass; Ubuntu-to-MSVC cross-build failed because `tauri-build-context.rs` was not generated and GNU/MSVC tooling was mismatched; native Windows prerequisite, exact-SHA CI, independent audit, and integration remain outstanding | Candidate `297e9b4...`; failed historical CI `32003632791`; local topology and fail-closed evidence tests pass; native `windows-tauri-build` is now the active qualification gate |
