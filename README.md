...
Accepted ADRs under `docs/decisions/` and `docs/adr/` preserve the architectural decision history. [`ADR-054`](docs/decisions/ADR-054-contract-consistency-and-schema-normalization.md) is the binding v1.0 consistency correction and SHALL be applied with the contract suite until those corrections are consolidated into a later clean contract revision.
